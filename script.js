const ApiKey = "7a38a8968dc04b968584c93b089b3de5";
...
const detail = "https://api.football-data.org/v2/teams/";
        ...
        .then(resJson => {
            ...
            resJson.teams.forEach(team => {
                ...
                <li class="collection-item avatar">
                    ...
                    <a href="#!" data-klep="${team.name}" data-id="${team.id}" class="secondary-content"><i class="material-icons" data-klep="${team.name}" data-id="${team.id}">info</i></a>
                </li>
                `
            });
            ...
            detail.forEach(btn => {
                btn.onclick = (event) => {
                    showTeamInfo(event.target.dataset.id, event.target.dataset.klep);
                }
            })
        }).catch(err => {
            console.error(err);
...
function showTeamInfo(id, klep, link) {
    let ling = detail + id;

    fetch(ling, fetchHeader)
        .then(response => response.json())
        .then(resJson => {
            console.log(resJson);
            let pemain = "";
            resJson.pemain.forEach(pemain => {
                pemain += `
                <li class="collection-item avatar">
                    <span class="title">${pemain.name}</span>
                    <p>Berdiri: ${pemain.nationality} <br>
                       Lokasi: ${pemain.position}
                       
                    </p>
                    <a href="#!" data-klep="${pemain.name}" data-id="${pemain.id}" class="secondary-content"><i class="material-icons" data-klep="${pemain.name}" data-id="${pemain.id}">info</i></a>
                </li>
                `
            })
        });
    contents.innerHTML = 'halo' + pemain;
    title.innerHTML = "Informasi " + klep;
}
